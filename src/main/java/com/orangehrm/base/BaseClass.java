...
public class BaseClass {
	...
	private synchronized void launchBrowser(String browser) {
		...
		if (browser.equalsIgnoreCase("chrome")) {
			...
			ChromeOptions options = new ChromeOptions();
			//options.addArguments("--headless=new"); // Run Chrome in modern headless mode (reliably respects window size)
			...
			getDriver().manage().window().setSize(new org.openqa.selenium.Dimension(1920, 1080)); // Explicit backup resize so the OrangeHRM sidebar never collapses into the hamburger menu
